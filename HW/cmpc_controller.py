...
def calc_Jacobian(x, u, param):
    ...
    A = np.array([
        [1, 0, -dt * v * np.sin(psi + np.arctan(L_r * np.arctan(delta) / (L_f + L_r))), dt * np.cos(psi + np.arctan(L_r * np.arctan(delta) / (L_f + L_r)))],
        [0, 1,  dt * v * np.cos(psi + np.arctan(L_r * np.arctan(delta) / (L_f + L_r))), dt * np.sin(psi + np.arctan(L_r * np.arctan(delta) / (L_f + L_r)))],
        [0, 0,  1, (dt * np.arctan(delta)) / (((L_r**2 * np.arctan(delta)**2) / (L_f + L_r)**2 + 1)**(0.5) * (L_r + L_f))],
        [0, 0,  0, 1]
    ])

    B = np.array([
        [0, -dt * L_r * v * np.sin(psi + np.arctan(L_r * np.arctan(delta) / (L_f + L_r))) / ((delta**2 + 1) * ((L_f + L_r)**2) / (L_f + L_r)** 2 + 1) * (L_r + L_f)],
        [0,  dt * L_r * v * np.cos(psi + np.arctan(L_r * np.arctan(delta) / (L_f + L_r))) / ((delta**2 + 1) * ((L_f + L_r)**2) / (L_f + L_r)** 2 + 1) * (L_r + L_f)],
...
def LQR_Controller(x_bar, u_bar, x0, param):
    ...
    P_state_combine = np.block([
        ...
        [np.zeros((dim_state, n_x - dim_state)), P_terminal]
    ])
